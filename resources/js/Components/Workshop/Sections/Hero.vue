<template>
  <section id="about"
    class="relative lg:min-h-screen pt-32 pb-20 px-6 overflow-hidden bg-gradient-to-b from-[#FDFCFE] to-[#F5F3FF] flex flex-col justify-center">
    <div class="container mx-auto">
      <div class="flex flex-col lg:flex-row items-center gap-16">
        <!-- Left Column -->
        <div v-motion :initial="{ opacity: 0, x: -50 }" :enter="{ opacity: 1, x: 0 }" class="flex-1 relative">
          <div
            class="inline-flex items-center gap-2 bg-violet-50 text-violet-700 px-4 py-1.5 rounded-full border border-violet-100 mb-6">
          ...
          <div class="h-12 mb-6">
            <transition name="typewriter" mode="out-in">
              <p :key="typewriterText" class="text-xl md:text-2xl font-bold text-violet-600">
                ⚡ {{ typewriterText }}
              </p>
            </transition>
          </div>
          ...
          <p class="text-lg text-slate-600 mb-10 max-w-xl leading-relaxed">
            Master practical AI techniques and transform your daily productivity — no coding required, no expensive
            tools, just skills you can use from day one.
          </p>
        ...
        <div v-motion :initial="{ opacity: 0, scale: 0.9, y: 50 }" :enter="{ opacity: 1, scale: 1, y: 0 }"
          class="flex-1 lg:max-w-md w-full">
          <Card class="relative p-8 border-2 border-violet-100 shadow-2xl bg-white/95">
            <div
              class="absolute -top-3 left-1/2 -translate-x-1/2 bg-violet-600 text-white px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg z-20">
              Highest Rated
            </div>
            ...
            <div class="flex items-center justify-between mb-6">
              ...
              <Badge variant="emerald"
                class="bg-emerald-50 text-emerald-600 border-none px-3 py-1 uppercase text-[10px] tracking-wider">50%
                OFF</Badge>
            </div>
            ...
            <div class="mb-6 p-3 bg-violet-50/50 rounded-xl border border-violet-100 flex items-center justify-between">
              <div class="flex items-center gap-2">
                ...
                <span class="text-[10px] font-black text-slate-600 uppercase tracking-wider">Only {{ stats.seats_left }}
                  seats left</span>
              </div>
            ...
            <Button variant="gradient" size="lg" class="w-full mb-4 group" @click="$emit('scrollToSection', 'pricing')">
              Register Now
              <ArrowRight class="w-6 h-6 ml-2 group-hover:translate-x-2 transition-transform" />
            </Button>
